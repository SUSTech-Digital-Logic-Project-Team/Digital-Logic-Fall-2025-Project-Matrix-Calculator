...
我们在顶层设计了一系列config寄存器，用于存储设置模式所设置的参数。设置模式允许用户通过UART自定义参数，包括最大矩阵维度、最大数值、同一维度最大矩阵数量、倒计时设置。FPGA复位后参数初始化为默认值，用户可在运行时修改。与周边模块的关系：设置模式与顶层模块交互，更新config寄存器；后续，顶层模块将这些参数传递给各子模块，如计算模式、生成模式等，用于限制输入范围和操作。
...
- 在开发和测试的两个环节中，我们通过AI生成重复的功能代码以及生成测试文件来进行开发。
...
来自项目仓库中README的部分，此部分对项目中大部分的代码细节进行了简要的解释，详见代码注释。
