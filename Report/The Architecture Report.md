...
- **后期阶段**：实现矩阵输入、存储、计算功能，发现综合问题和时序违例，重构为BRAM存储。
...
总而言之，我们在本次的项目开发中学到了如何搭建一个完整的小项目，并且从AI的代码中学习了很多Verilog相关的知识（例如，强制综合器使用BRAM，强制综合器不优化掉某一个变量），同时也学会了很多Debug方面的知识，例如编写测试文件，设置断电测试并上板通过串口进行调试，等等。最终我们也顺利完成了本次Project的开发，实现了绝大部分的功能，并且实现了参数设置、输出对齐和卷积等bonus功能。
